  ...
  begin
    ...
    for I := 0 to PropCnt-1 do
      ...
      case PropList[I].PropType^^.Kind of
        ...
        tkInt64 :
          begin
            Int64Value := GetOrdProp(Obj, PropList[I]);
            if Int64Value <> 0 then
              Attrs.addAttribute('', '', CamelToXml( string(PropList[I].Name), Camelize), '', IntToStr(Int64Value));
          end;
        tkEnumeration:
          begin
            IntValue := GetOrdProp(Obj, PropList[I]);
            if IntValue < 0 then
            begin
              if IntValue < GetTypeData(PropList[I].PropType^).MinValue then
                IntValue := GetTypeData(PropList[I].PropType^).MinValue;
            end;
...
//    raise ERttiBindException.CreateFmt('Cannot bind XML node with path %s on attributes %s', [ObjectPath, AttrName]);
    ...
    case PropInfo.PropType^.Kind of
      ...
      begin
        ...
        begin
          if not(TryStrToInt(AttrValue, iValue)) then
          ...
          begin
            ...
            if typedata = nil then
              iValue := 0
            else if iValue < typedata.MinValue then
              iValue := typedata.MinValue;
          end;
          SetOrdProp(FCurrentObject, PropInfo, iValue);
        end;
      end;
      tkInt64 : SetOrdProp(FCurrentObject, PropInfo, StrToInt64(AttrValue));
      {$ifdef COMPILER_12_UP}
      tkUString,
      {$endif}
      tkString,
